...
class FrontEnd(object):
    ...
    lp = Literal('(').suppress()
    rp = Literal(')').suppress()
    table = CaselessKeyword('table').suppress() + Word(alphas).setResultsName('table')
    ...
    opmode = Optional(CaselessKeyword('optional').setResultsName('is_optional'))
    cvmode = Optional(CaselessKeyword('const').setResultsName('is_const'))
    mode = opmode & cvmode
    column = CaselessKeyword('column').suppress() + \
             Group(Word(alphas).setResultsName('name') + datatype + mode).setResultsName('column')
...
class Validator(object):
    ...
    class NonUniqueColumnNames(Error):
        ...
        def __init__(self, msg):
            super(Validator.NonUniqueColumnNames, self).__init__('multiple definitions of %s!'%str(msg))


    class NoColumnsProvided(Error):

        def __init__(self):
            super(Validator.NoColumnsProvided, self).__init__('no column definitions provided!')
